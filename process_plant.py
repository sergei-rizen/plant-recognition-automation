...
def get_name_from_text_hint(text_hint):
    """Uses Gemini Pro (text-only) to correct a plant name hint."""
...
def get_name_from_image(image_data):
    """Uses Gemini Pro Vision as a last resort to identify the plant from the image."""
...
def main():
    ...
    try:
        ...
        # Determine the initial text hint from either filename or OCR
        text_source = None
        ...
        # Step 1: Use the text hint (if we have one) with the text-only AI model
        if text_source:
            final_result_string = get_name_from_text_hint(text_source)

        # Step 2: If the AI failed on the text hint, fall back to PlantNet
        ...
        # Step 3: If PlantNet also failed, use Gemini Vision as the final resort (Scenario C)
        if not final_result_string:
    ...
    # Ensure there is always a result to report
    if not final_result_string:
