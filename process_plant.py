...
import requests
import json
import base64 # Needed to encode the image
...
def is_valid_plant_name(text):
    if not text or len(text) < 3 or len(text) > 100: return False
    text_lower = text.lower()
    gibberish_patterns = ['screenshot', 'camera', 'photo']
    if any(pattern in text_lower for pattern in gibberish_patterns): return False
    return True

def identify_plant_with_plantnet(image_data):
    api_key = os.environ['PLANTNET_API_KEY']
    api_url = "https://my-api.plantnet.org/v2/identify/all"
    params = {'include-related-images': 'false', 'no-reject': 'false', 'nb-results': '10', 'api-key': api_key}
...
def main():
    ...
    try:
        ...
        if is_valid_plant_name(clean_text):
            ...
            final_result = {'source': 'ocr_google_vision', 'plant_name': clean_text, 'image_id': image_id}
        else:
            ...
            if plantnet_result.get('results'):
                ...
                final_result = {'source': 'plantnet','plant_name': best_match['species']['scientificNameWithoutAuthor'],'confidence': best_match['score'],'common_names': best_match['species'].get('commonNames', []),'image_id': image_id}
            else:
    ...
    with open("result.json", "w") as f: f.write(result_json)
    with open("result.log", "w") as f: f.write(result_json)
